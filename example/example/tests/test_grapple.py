...
class PageUrlPathTest(BaseBifrostTest):
...
class SitesTest(TestCase):
    def setUp(self):
        self.site = wagtail_factories.SiteFactory(
            hostname="bifrost.localhost", site_name="Bifrost test site"
        )
        self.client = Client(SCHEMA)
...
@override_settings(BIFROST_AUTO_CAMELCASE=False)
...
class ImagesTest(BaseBifrostTest):
    def setUp(self):
        super().setUp()
...
class DocumentsTest(BaseBifrostTest):
    def setUp(self):
        super().setUp()
...
class MediaTest(BaseBifrostTest):
    def setUp(self):
        super().setUp()
